    ...
    def handle_client(self):
        ...
        try:
            ...
            with  global_setting.get_setting("condition"):
                # 接收到了数据
                global_setting.get_setting("data_buffer").append(1)
                logger.debug(f"data_buffer - 加1-长度{len(global_setting.get_setting('data_buffer'))}")
                # 如果所有线程都发送完数据，通知处理线程
                if len(global_setting.get_setting("data_buffer")) == int(
                    ...
                    global_setting.get_setting("condition").notify()  # 通知处理线程开始处理
                pass
            pass
        except Exception as e:
